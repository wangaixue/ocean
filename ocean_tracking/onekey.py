...
def main():
    ...
    if trainINFO['ISTRUE']:
        print('==> train phase')
        print('python ./ocean_tracking/train_ocean.py --cfg {0} --gpus {1} --workers {2} 2>&1 | tee logs/ocean_train.log'
                  .format(args.cfg, info['GPUS'], info['WORKERS']))

        if not exists('logs'):
            os.makedirs('logs')

        os.system('python ./ocean_tracking/train_ocean.py --cfg {0} --gpus {1} --workers {2} 2>&1 | tee logs/siamrpn_train.log'
                  .format(args.cfg, info['GPUS'], info['WORKERS']))

    # epoch testing -- test 30-50 epochs (or more)
    if testINFO['ISTRUE']:
        print('==> test phase')
        print('mpiexec -n {0} python ./ocean_tracking/test_epochs.py --arch {1} --start_epoch {2} --end_epoch {3} --gpu_nums={4} \
                  --threads {0} --dataset {5}  --RGBTSPLIT {6} 2>&1 | tee logs/ocean_epoch_test.log'
        ...
        os.system('mpiexec -n {0} python ./ocean_tracking/test_epochs.py --arch {1} --start_epoch {2} --end_epoch {3} --gpu_nums={4} \
                  --threads {0} --dataset {5}  --RGBTSPLIT {6} 2>&1 | tee logs/ocean_epoch_test.log'
                  .format(testINFO['THREADS'], trainINFO['MODEL'], testINFO['START_EPOCH'], testINFO['END_EPOCH'],
                          (len(info['GPUS']) + 1) // 2, testINFO['DATA'], testINFO['RGBTSPLIT']))
    ...
    if tuneINFO['ISTRUE']:
        ...
        print('==> tune phase')
        print('python -u ./ocean_tracking/tune_tpe.py --arch {0} --resume {1} --dataset {2} --gpu_nums {3} \
                  2>&1 | tee logs/tpe_tune.log'.format(trainINFO['MODEL'], 'snapshot/'+ resume, tuneINFO['DATA'], (len(info['GPUS']) + 1) // 2))

        if not exists('logs'):
            os.makedirs('logs')
        os.system('python -u ./ocean_tracking/tune_tpe.py --arch {0} --resume {1} --dataset {2} --gpu_nums {3}\
                  2>&1 | tee logs/tpe_tune.log'.format(trainINFO['MODEL'], 'snapshot/'+ resume, tuneINFO['DATA'], (len(info['GPUS']) + 1) // 2))
